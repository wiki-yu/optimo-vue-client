                ...
                <div class="row">
                  ...
                  <div class="col">
                    <svg id="i-chevron-left" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" width="32"
                      ...
                      stroke-width="2" style="cursor: pointer;" @click="previousFrame">
                  ...
                  <div class="col">
                    ...
                    <svg id="i-play" v-else xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" width="32"
                      height="32" fill="none" stroke="currentcolor" stroke-linecap="round" stroke-linejoin="round"
                      stroke-width="2" style="cursor: pointer;" @click="playPause">
              ...
              <table class="table table-striped" id="table" ref="table">
                ...
                <tbody id=table-body v-for="boundingBox in boundingBoxes" :key="boundingBox.row" :row="boundingBox.row">
                  <tr ref="row-key" class="row-unsaved">
                    ...
                    <td ref="activity-key">
                      <input ref="text-input-key" class="activity" type="text" :value="`Activity ${boundingBox.row}`">
                    </td>
                    ...
                    <td ref="row-hand-key">
                      <div class="custom-control custom-checkbox">
                        <input type="checkbox" class="custom-control-input" :id="`right-hand-${boundingBox.row}`" value="-1"
                         @click="checkBox(`${boundingBox.row}`, 'right')"
                         ref="right-hand-key">
                        <label class="custom-control-label" :for="`right-hand-${boundingBox.row}`">Right Hand</label>
                      </div>
                      <div class="custom-control custom-checkbox">
                        <input type="checkbox" class="custom-control-input" :id="`left-hand-${boundingBox.row}`" value="-1"
                         @click="checkBox(`${boundingBox.row}`, 'left')"
                         ref="left-hand-key">
                        <label class="custom-control-label" :for="`left-hand-${boundingBox.row}`">Left Hand</label>
                      </div>
                    </td>
                    <td class="bounds">
                    ...
                    <td ref="button-key">
                      <button class="btn btn-dark btn-sm"
                        @click="updateButton(`${boundingBox.row}`, $event)">
                        Save
                      </button>
                      <br><br>
                      <button class="btn btn-dark btn-sm"
                        @click="deleteButton(`${boundingBox.row}`, $event)">
                        Delete
                      </button>
                    </td>
                  </tr>
  ...
  methods: {
    ...
    downloadTable: function () {
      ...
      for (var i = 0; i < rows.length; i++) {
        ...
        var row = [];
        row.push(tds[0].getElementsByTagName('input')[0].value);
        row.push(tds[1].getElementsByTagName('input')[0].value);
        row.push(tds[2].getElementsByTagName('select')[0].value);
        row.push(tds[3].getElementsByTagName('input')[0].value);
        row.push(tds[4].getElementsByTagName('input')[0].value > 0);
        row.push(tds[4].getElementsByTagName('input')[1].value > 0);
        row.push(this.boundingBoxes[i].x1)
        row.push(this.boundingBoxes[i].y1)
        row.push(this.boundingBoxes[i].x2)
        row.push(this.boundingBoxes[i].y2)
        //row.push(tds[5].innerHTML); // bounds
        row.push(tds[6].innerHTML);
        row.push(tds[7].innerHTML);
        row.push(tds[8].getElementsByTagName('select')[0].value);
        csv.push(row.join(','));
      }
